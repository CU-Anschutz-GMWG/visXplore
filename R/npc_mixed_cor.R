...
#'
#' @param cor_results a visx_cor object or list from pairwise_cor output
...
npc_mixed_cor <- function (cor_results,
  ...
  # Build edge data.frame using vectorized indexing
  if(n_paths > 0){
    idx <- which(!is.na(proximity), arr.ind = TRUE)
    paths <- data.frame(
      x = points$x[idx[, 1]],
      y = points$y[idx[, 1]],
      xend = points$x[idx[, 2]],
      yend = points$y[idx[, 2]],
      proximity = proximity[idx],
      sign = sign(cor_results$cor_value[idx]),
      type = cor_results$cor_type[idx],
      stringsAsFactors = FALSE
    )
    paths <- paths %>% mutate(across(-type, as.numeric))
  }
  ...
  if(show_signif){
    ...
    if(n_paths2 > 0){
      idx2 <- which(!is.na(proximity2), arr.ind = TRUE)
      paths2 <- data.frame(
        x = points$x[idx2[, 1]],
        y = points$y[idx2[, 1]],
        xend = points$x[idx2[, 2]],
        yend = points$y[idx2[, 2]],
        proximity = proximity2[idx2]
      )
    }
  }

  # plot: start with variable points
  npc <- ggplot()+
  ...
  # add correlation edges (only if any exist)
  if(n_paths > 0){
